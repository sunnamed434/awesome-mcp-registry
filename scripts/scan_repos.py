...
# EU AI Act Art. 50 (applies from 2026-08-02): every bot comment carries a
# visible disclosure for humans plus a machine-readable marker for tools.
# Two variants because readers weigh them differently: LLM-written verdicts
# vs fixed template text.
_OWNER = REPO_SLUG.split("/")[0]
AI_FOOTER = (
    f"\n\n---\n*This comment was generated by AI (an automated LLM pipeline, no human "
    f"pre-review) and may contain mistakes. To appeal or reach a human, reply here or "
    f"mention @{_OWNER}.*\n"
    f"<!-- ai-generated: true; source: llm; workflow: auto-scanner -->")
BOT_FOOTER = (
    f"\n\n---\n*Automated comment from this repository's workflows — not written by a "
    f"human. Questions or appeals: reply here or mention @{_OWNER}.*\n"
    f"<!-- ai-generated: false; source: template; workflow: auto-scanner -->")
...
def analyze_with_ai(repo_data, readme_content, mcp_signals=None, max_retries=3,
    ...
    Every call runs at temperature 0 — DeepSeek's maximum-consistency setting for
    structured judging. Not byte-deterministic (the API has no seed parameter),
    which is why canaries check envelopes, not exact matches.
    """
    ...
    payload = {
        ...
        # V4 models think by default. Thinking would eat max_tokens with
        # reasoning (HTTP 200 but an empty/truncated verdict) AND silently
        # ignore temperature — the consistency control this judge depends on.
        # Rubric classification is DeepSeek's own "non-thinking" use case.
        "thinking": {"type": "disabled"},
        # Belt and braces with the prompt's schema: json_object guarantees
        # syntactically valid JSON (the prompt contains the required word
        # "JSON" and an example shape).
    ...
    try:
        for attempt in range(max_retries):
            ...
            if not (content or "").strip() and attempt < max_retries - 1:
                # Documented JSON-mode caveat: the API "may occasionally
                # return empty content" — retry instead of failing the repo.
                print("    (empty content from AI API; retrying)")
                time.sleep(5)
                continue
...
def post_issue_comment(issue_number, body, ai=False):
    """Comment on an issue, appending the disclosure footer (`ai=True` marks
    LLM-generated text vs a fixed template). Read-only (prints intent) outside
    GitHub Actions."""
    body += AI_FOOTER if ai else BOT_FOOTER
    if not IS_CI:
...
def process_nominations(cache, excluded, listed_for_squat):
    ...
    for nom in nominations[:MAX_NOMINATIONS]:
        ...
        notify(num, author, verdict_comment(fn, analysis, trust_obj,
                                            repo_id=entry.get("repo_id")), ai=True)
        mark_done(num, rl)
        close_issue(num, close_reason_for(rl))
        processed += 1
        time.sleep(4)
