...
Every automated comment in this repository is labeled (per EU AI Act Art. 50): a visible footer
states whether the text was **generated by AI** (LLM-written verdicts) or is a **fixed workflow
template**, and the raw comment body carries a machine-readable marker —
`<!-- ai-generated: true|false; source: llm|template; workflow: ... -->` — so tools can detect
bot-authored content via the API.
